...
def construct_DPE(src, dst, mask, index_map, coordinate_map, mixing=False):
    ...
    for i in range(N):
        ...
        ### comment the following lines to see the simplest interpolation
        ### tricky data type
        if not mixing: 
...
output: xr, xg and xb (solutions of the equation)
'''
...
def solve_DPE(A,b):
    ...
    xr,_ = linalg.cg(A, b[:, 0]) # https://docs.scipy.org/doc/scipy/reference/generated/scipy.sparse.linalg.cg.html
